...
int main(int argc,char *argv[]){    
	...
	for(int i = 0; i < 10; ++i){
	    ...
	    std::cout << "x: " << x << " y: " << y << " attitude: " << attitude << std::endl;
		psuedo_path.push_back(CoordinateAttitude{r2d2::Coordinate(x * r2d2::Length::METER, y * r2d2::Length::METER, 0 * r2d2::Length::METER), Attitude(0 * r2d2::Angle::rad, 0 * r2d2::Angle::rad, attitude * r2d2::Angle::rad)});
	}
    
	CoordinateAttitude status = {r2d2::Coordinate(0 * r2d2::Length::METER, 0 * r2d2::Length::METER, 0 * r2d2::Length::METER), Attitude()};
    ...
    r2d2::PilotSimulation pilot_simulation(robot_status, speed, rotation_speed, waypoint);
	
    std::thread pilot_simulation_thread(&r2d2::PilotSimulation::run, &pilot_simulation);
